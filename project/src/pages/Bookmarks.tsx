    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] transition-colors duration-300">
            ...
            <main className="max-w-6xl mx-auto px-6 py-10">
                ...
                <Card className="p-4 mb-6 bg-[#FCFAF8] border-none shadow-xl rounded-3xl">
                ...
                {filtered.length > 0 ? (
                    <div className="space-y-4">
                        {filtered.map((item) => (
                            <Card key={item.id} className="p-6 bg-[#FCFAF8] border-none shadow-xl rounded-3xl">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="px-2 py-1 bg-[#8C5A35]/10 text-[#8C5A35] text-[10px] font-black rounded-lg uppercase tracking-wider">
                                                {item.type}
                                            </span>
                                            <span className="text-xs text-stone-500">{item.subject} • {item.chapter}</span>
                                        </div>
                                        {item.question && (
                                            <p className="text-sm font-bold text-[#2C241B] mb-2">Q: {item.question}</p>
                                        )}
                                        <div className="mt-1">
                                            <p className={`text-sm text-stone-600 leading-relaxed whitespace-pre-wrap ${expandedId === item.id ? '' : 'line-clamp-5'}`}>
                                                {item.answer}
                                            </p>
                                            <Button
                                                variant="ghost"
                    ...
                    <Card className="p-14 bg-[#FCFAF8] border-none shadow-xl rounded-3xl text-center">
                        <Bookmark className="mx-auto text-[#8C5A35] mb-3" size={28} />
                        <p className="font-semibold text-[#2C241B] ">No saved items yet.</p>
                        <p className="text-sm text-stone-500 mt-1">Save useful answers and flashcards to revise later.</p>
                    </Card>
                )}
            </main>
                            ...
                            {activeBookmark.type === 'flashcard' ? (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-xs font-black uppercase tracking-wider text-[#8C5A35]">flashcard</p>
                                            <h4 className="text-lg font-black text-[#2C241B] ">{activeBookmark.subject} • {activeBookmark.chapter}</h4>
                                        </div>
