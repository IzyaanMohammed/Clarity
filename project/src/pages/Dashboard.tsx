...
import { BookOpen, Target, Flame, Clock, ArrowRight, Brain, Star, History, Calendar, Zap, Award, Sparkles } from 'lucide-react';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="p-8 bg-white dark:bg-[#0f172a] border-none shadow-xl rounded-[32px] relative overflow-hidden group">
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-slate-500 mb-6 leading-relaxed">System detected focus on **{user.subjects[0]}**. Recommended next: **{user.subjects[1] || 'Practice Test'}**.</p>
                ...
                <Card className="p-8 bg-[#1D9E75] text-white border-none shadow-xl rounded-[32px] relative overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        try {
                          const data = await (await fetch(`http://localhost:8000/api/v1/progress/report/${user.name}`)).json();
                          alert(data.report);
                        } catch (e) {
                          toast.error("Report System Offline");
                        }
                      }}
                    >
                      Send Weekly Parent Report
                    </Button>
                  </div>
              ...
              <Card className="bg-white dark:bg-[#0f172a] border-none shadow-xl rounded-[32px] overflow-hidden">
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {stats?.recent_activity?.length > 0 ? (
                    [...stats.recent_activity].reverse().slice(0, 5).map((activity: any, index: number) => (
                      <div key={index} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex items-center justify-between">
                        <div className="flex items-center gap-5">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                            activity.action === 'question' ? 'bg-purple-100 text-purple-600' : 'bg-emerald-100 text-emerald-600'
                          }`}>
              ...
              <Card className="p-8 bg-slate-900 text-white border-none rounded-[32px] shadow-2xl relative overflow-hidden group">
                <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm mb-8 leading-relaxed">
                    Unlock Unlimited Vision AI, handwritten notes scanner, and Mock Exams for <span className="text-white font-bold">50 AED/mo.</span>
                  </p>
                  <Button 
