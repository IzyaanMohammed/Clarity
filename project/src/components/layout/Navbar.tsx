...
export const Navbar = () => {
  ...
  const renderLink = (link: { to: string; label: string; icon: typeof BookOpen }) => {
    ...
    return (
      ...
      >
        <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${active ? 'bg-[#FCFAF8]/15' : 'bg-[#F2EFE9] '} transition-colors`}>
  ...
  return (
    <>
      <nav className={`hidden md:flex fixed left-0 top-0 bottom-0 ${collapsed ? 'w-20' : 'w-72'} bg-[#FCFAF8]/95 backdrop-blur-xl border-r border-stone-200 z-50 transition-all duration-300`}>
        <div className={`relative flex h-full w-full flex-col ${collapsed ? 'p-3' : 'p-4'}`}>
          ...
          <div id="sidebar-scroll-container" onScroll={handleScroll} className="mt-5 space-y-5 overflow-y-auto pr-1">
            <section>
              {!collapsed && <p className="px-3 text-[10px] font-black uppercase tracking-[0.24em] text-stone-400 ">Home</p>}
            ...
            <section>
              {!collapsed && <p className="px-3 text-[10px] font-black uppercase tracking-[0.24em] text-stone-400 ">NCERT Study</p>}
              <div className="mt-2 space-y-1">
            ...
            <section>
              {!collapsed && <p className="px-3 text-[10px] font-black uppercase tracking-[0.24em] text-stone-400 ">Practice</p>}
              <div className="mt-2 space-y-1">
            ...
            <section>
              {!collapsed && <p className="px-3 text-[10px] font-black uppercase tracking-[0.24em] text-stone-400 ">Clarifier AI</p>}
              <div className="mt-2 space-y-1">
            ...
            <section>
              {!collapsed && <p className="px-3 text-[10px] font-black uppercase tracking-[0.24em] text-stone-400 ">Tools</p>}
              <div className="mt-2 space-y-1">
              ...
              >
                ...
                {!collapsed && (
                  <div className="min-w-0">
                    <p className="truncate text-sm font-black text-[#2C241B] ">{user.name}</p>
                    <p className="text-xs font-medium text-stone-500">Class {user.class}</p>
                  </div>
                )}
              </button>
            )}
